...
\paragraph{Strong claim (statistically significant, 4 of 6
contrasts).} Both event-time policies T1 and T2 strongly outperform
passive holds of \textbf{Aave V3, Compound V3, Morpho Blue, and
Fluid} in all $\WFNWindows$ walk-forward windows
(T1 vs Aave $\widehat{\Delta\text{APY}} = +1.84$\,pp, $p < 10^{-4}$;
T1 vs Compound $+1.65$\,pp, $p < 10^{-4}$; T1 vs Morpho $+1.61$\,pp,
$p < 10^{-4}$; T1 vs Fluid $+1.65$\,pp, $p < 10^{-4}$).  These four
protocols together represent $\sim\$30$\,B addressable TVL of the
design universe.

\paragraph{Mid-strength claim (5/6 wins, p = 0.03--0.08).}
T1 beats passive Spark 5/6 windows ($+0.80$\,pp, $p = 0.034$);
T2 the same 5/6 ($+0.60$\,pp, $p = 0.084$).  Spark is a younger
Aave-V3-fork whose supply rate occasionally spikes above the
aggregate active allocator; the directional consistency is high
but the per-window margin is small.

\paragraph{Honest gap (Euler V2).} From window W3 onward Euler V2
has been the single top-yielding protocol on the panel; no F3-only
policy preferentially routes to it (T1 $+0.48$\,pp, $p = 0.198$;
T2 $+0.28$\,pp, $p = 0.261$).  Closing this gap requires the F1
lead-rate signal (Maker DSR, Curve 3pool, Euler-specific leads),
which is the explicit journal-extension scope.
