...
2026 ($\NBlocks$ blocks).  On the walk-forward $N\!=\!6$ paired
bootstrap (the binding inference) T1 outperforms each in-scope
passive hold by $+1.5$ to $+2.8$ percentage points ($p<10^{-4}$ on
5 of 6 contrasts; \Cref{tab:wf-nxm}); the sophisticated-retrain T3
(Cox proportional-hazards on the F1$+$F3 design matrix) closes
pre-registered H1c with $\Delta\textsc{APY} = +7.03$\,bp over T1
($p = 0.015$).  The secondary monthly Sharpe bootstrap on the test
slice is consistent in direction but underpowered at $N=4$
observations (\Cref{tab:secondary-monthly}).  Two orders of
magnitude more rebalance opportunities than the hourly baseline of
