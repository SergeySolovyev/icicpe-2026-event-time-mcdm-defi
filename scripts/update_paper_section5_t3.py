...
def _build_paragraph() -> str:
    ...
    para = (
        ...
        "(López de Prado AFML Ch.4.5), L2 ridge sweep. Ablations land "
        "OOF C-index of $0.516$ (F3 only), $0.531$ (F1+F3), $0.560$ "
        "(F3+F4), $0.549$ (canonical F1+F3+F4); the top non-F3 "
        "coefficient is $f1\\_dsr\\_delta\\_300$ ($\\beta = +15.29$, "
        "one-hour change in Maker DSR), direct empirical support for "
        "the F1 lead-rate signal class. In walk-forward T3 yields "
        f"$\\Delta\\textsc{{APY}} = \\TThreeMeanDeltaBp$\\,bp "
        f"(95\\% paired-bootstrap CI "
        f"$[\\TThreeCILow, \\TThreeCIHigh]$, $p = \\TThreePValue$, "
        f"$\\TThreeWins/\\TThreeNWindows$ windows positive) over T1, "
        f"{h1c_status}. The full ablation table + per-window head-to-"
        "head lives in the institutional dossier §02.\n\n"
    )
