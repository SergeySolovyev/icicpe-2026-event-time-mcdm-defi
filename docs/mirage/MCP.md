...
This demonstrates working MCP discovery, saved evidence replay and access to the existing allocator/gate, including the size-mismatch case. By default it uses no network and does not run an LLM or submit a transaction. The default snapshot follows `mirage/snapshots/demo.json`; its results can change if that artifact changes. SDK context managers shut down the local stdio child on completion or failure. The example prints safe tool error codes/stages and returns a nonzero exit code on failure. Raw exception text stays suppressed, but the stdio child's own error output is written to `mirage_mcp_server_stderr.log` in the system temporary directory and its tail is printed when the run fails, so a server that dies during startup is distinguishable from a slow one.
...
`--live` adds one `inspect_market` call after the saved replay, so a reviewer can watch Graph discovery gate a live capture from a single command instead of configuring a GUI client. With no argument it inspects the saved demo's WETH market; pass a different 32-byte Morpho ID to inspect another. The server resolves a finalized block, requires that market to appear in live Graph discovery at the matching block hash, and only then reads contracts; a market absent from discovery returns `market_not_discovered` before any contract call. This is the only networked part of the example, it can take minutes, and a failure is reported without invalidating the saved replay above it. The captured block is newer than the saved evidence, so its verdict may legitimately differ; a live capture never replaces the saved report.

Observed on 11 September 2026 on the Windows development host, both outcomes in the same hour. One run returned `live_inspection_failed` at stage `full_capture` and the saved replay printed above it was unaffected. A second run completed: `mode=live_capture`, finalized block `25952011`, hash `0xb426aaf920978befbee5d1e77c333ea696f71d06615969bef0338154e3dc5b30`, 699 USDC markets discovered through The Graph, one inspected, severity `pass` with four findings. Discovery returning 699 rather than the 697 of the saved capture is expected: new markets are created as the chain advances. The live path depends on free public RPC endpoints without an API key, so a transient failure is a normal outcome rather than a defect, and it is reported as its own stage instead of degrading into a passing verdict.

### Timeouts

The stdio child imports the scientific stack before answering `initialize`. On a loaded Windows host on 11 September 2026, importing `mirage.mcp_server` alone measured 48.9 seconds and a completed handshake 178.8 seconds, so the client read timeout defaults to 300 seconds and is raised with `--timeout` or `MIRAGE_MCP_TIMEOUT`. A timeout shorter than the worst observed cold start turns a slow start into a failure; an earlier 60-second default did exactly that.
...
The exact portable-launcher commit `c8092dfe0c5e4136e3530d835ec6e3bc07245f55` then passed [CI run 34346725909](https://github.com/SergeySolovyev/icicpe-2026-event-time-mcdm-defi/actions/runs/34346725909) on Ubuntu 24.04/Python 3.12: **219 tests passed, two warnings, 8.54 seconds**, including both fresh-process launcher variants. The warnings identify the intentionally absent research packages `torch` and `onnxruntime`. The official SDK example separately reproduced all three allocation scenarios above at block `25938082` and closed its session. The Docker job built successfully and replayed four saved markets with network access disabled. These checks used saved evidence; they did not repeat live Graph or mainnet capture.
