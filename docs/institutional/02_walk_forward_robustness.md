...
## Scope: 6 protocols + 3 policy tiers

The Vol-2 N×M matrix covers the **full 6-protocol design universe**
on Ethereum L1 USDC supply markets: **Aave V3** (~36% TVL share),
**Compound V3** (~5%), **Spark / SparkLend** (~13%), **Morpho
Blue** (~9%), **Euler V2** (~2%), **Fluid Finance** (~3%). Together
these represent ~$36B of the $54B Ethereum-L1 USDC-lending TVL, or
~67% of the design universe. Data sources by protocol are tabulated
in §I (the per-block panel handles Aave/Morpho/Euler; Compound is
re-built from a verified hourly RPC parquet; Spark via the Sky/Spark
Messari subgraph; Fluid via DeFiLlama Yield-Pools daily APY).
