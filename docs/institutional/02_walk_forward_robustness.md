...
| Features      | OOF C-index | Δ vs F3-only |
|---------------|------------:|-------------:|
| F3 only       | 0.516       | baseline     |
| F1 + F3       | 0.531       | +1.5 pp      |
| F3 + F4       | 0.560       | +4.4 pp      |
| **F1+F3+F4**  | **0.549**   | **+3.3 pp**  |

The canonical F1+F3+F4 model promoted to production has top-five
coefficients led by `f3_dispersion_std` (β = +73.99), then
`f1_dsr_delta_300` (+15.29) — the 1-hour change in Maker DSR is
the strongest non-F3 predictor, providing direct empirical
support for the MacKenzie F1 signal class in DeFi-lending context.
