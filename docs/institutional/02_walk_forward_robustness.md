...
T3 has been retrained on the full F1+F3+F4 design matrix with
López de Prado AFML methodology: triple-barrier labels (24-h
horizon), purged 5-fold cross-validation with embargo = 5.4 days,
sample-uniqueness weighting (Ch.4.5), and L2 ridge penalizer sweep.
Four feature ablations were tested; OOF C-index ranking:

| Features         | OOF C-index | Δ vs F3-only | Add value? |
|------------------|------------:|-------------:|-----------:|
| F3 only          | 0.563       | baseline     | —          |
| F1 + F3          | **0.582**   | **+1.9 pp**  | ✓ F1 helps |
| F3 + F4          | 0.563       | 0.0 pp       | ✗ F4 zero  |
| **F1 + F3 + F4** | **0.582**   | **+1.9 pp**  | F4 zero    |

**Honest negative finding for F4**: the F4 related-instruments class
(USDC peg deviation, ETH/USD) adds **zero** concordance above F3-only
after the gas placeholder (constant 25 gwei) is dropped as zero-
variance. Only F1 (Maker DSR one-hour delta, β = +15.29) provides
predictive lift above F3 fragmentation features. This refines the
MacKenzie (2021) Table 3.2 mapping to DeFi-lending: **F1 is the
operative non-F3 signal class** for cross-protocol leader timing on
this panel; F4 contribution is queued for re-test with real historical
gas prices (Etherscan key).
...
| **T3** Cox hazard | **+2.88pp**<br>p=0.0000<br>6/6 | **+2.70pp**<br>p=0.0000<br>6/6 | **+1.85pp**<br>p=0.0000<br>6/6 | **+2.65pp**<br>p=0.0000<br>6/6 | **+1.53pp**<br>p=0.0114<br>5/6 | **+2.70pp**<br>p=0.0000<br>6/6 |
